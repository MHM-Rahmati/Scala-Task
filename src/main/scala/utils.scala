...
object utils {
  ...
  def jreader(fileName: String, dataType: String) = {
    val jsonString = os.read(os.pwd / "data" / "input" / fileName)
  ...
  def saveListAsJson(list: scala.collection.immutable.Iterable[Map[String, Matchable]], filename: String): Unit = {
    val pwd = os.pwd
    val fullFileName = s"$pwd/data/output/$filename"
    implicit val formats: Formats = DefaultFormats
    val jsonString = write(list)
    Using(Files.newBufferedWriter(Paths.get(fullFileName))) { writer =>
